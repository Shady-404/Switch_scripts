...
try:
    ...
    acl_entries = []
    print("Enter ACL entries (e.g., 'permit 192.168.1.0 0.0.0.255'). Press 'Enter' to finish!")
    while True:
        entry = get_user_input("Enter an ACL entry: ")
        if entry.strip() == '':
            break
        acl_entries.append(f"ip access-list standard {acl_name}")
        acl_entries.append(f"{entry}")
    ...
    print(final_output)
    print(f"Standard named ACL '{acl_name}' configured and applied to interface {interface} {direction} successfully")
