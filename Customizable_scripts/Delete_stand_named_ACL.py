...
try:
    ...
    acl_name = get_user_input("Enter the standard ACL name to delete: ")

    # Command to delete the entire ACL
    delete_acl_command = f"no ip access-list standard {acl_name}"
    ...
    if '% Invalid input detected' in output or '% Incomplete command' in output:
        ...
        print(f"Standard named ACL '{acl_name}' deleted successfully")
